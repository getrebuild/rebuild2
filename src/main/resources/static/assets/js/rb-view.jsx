                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            ...
                            <p className="text-muted">无可用报表 {rb.isAdminUser &&
                            <a className="icon-link ml-1" target="_blank"
                               href={`${rb.baseUrl}/admin/datas/data-reports`}><i
                            ...
                            <div>
                                <ul className="list-unstyled">
                                    {(this.state.reports || []).map((item) => {
                                        ...
                                        return <li key={'r-' + item.id}><a target="_blank" href={reportUrl}
                                                                           className="text-truncate">{item.name}<i
                                            className="zmdi zmdi-download"></i></a></li>
                                    })}
                                </ul>
                            </div>
                        </div>
    ...
    render() {
        return (
            <div className={`related-list ${!this.state.list ? 'rb-loading rb-loading-active' : ''}`}>
                ...
                <div className="list-nodata"><span className="zmdi zmdi-info-outline"/><p>暂无相关数据</p></div>}
                {(this.state.list || []).map((item) => {
                    return <div className={`card ${this.state.viewOpens[item[0]] ? 'active' : ''}`}
                                key={`rr-${item[0]}`}>
                        <div className="row header-title" onClick={() => this._toggleInsideView(item[0])}>
                            <div className="col-10">
                                <a href={`#!/View/${this.props.entity.split('.')[0]}/${item[0]}`}
                                   onClick={(e) => this._handleView(e)} title="打开">{item[1]}</a>
                            </div>
                ...
                {this.state.showMores && (
                    <div className="text-center load-mores">
                        <div>
                            <button type="button" className="btn btn-secondary" onClick={() => this.fetchList(1)}>加载更多
                            </button>
                        </div>
                    </div>
                )}
    ...
    render() {
        return (
            <div className={`related-list ${!this.state.data ? 'rb-loading rb-loading-active' : ''}`}>
                ...
                <div className="list-nodata"><span className="zmdi zmdi-chart-donut"/><p>暂无相关跟进</p></div>}
                ...
                && <div className="text-center load-mores">
                    <div>
                        <button type="button" className="btn btn-secondary" onClick={() => this.fetchFeeds(1)}>加载更多
                        </button>
                    </div>
                </div>}
            </div>
...
const RbViewPage = {
    ...
    init(id, entity, ep) {
        ...
        $('.J_edit').click(() => RbFormModal.create({
            id: id,
            title: `编辑${entity[1]}`,
            entity: entity[0],
            icon: entity[2]
    ...
    initRecordMeta() {
        $.get(`/app/entity/record-meta?id=${this.__id}`, (res) => {
            ...
            for (let k in res.data) {
                ...
                if (k === 'owningUser') {
                    renderRbcomp(<UserShow id={v[0]} name={v[1]} showName={true} deptName={v[2]}
                                           onClick={() => this.clickViewUser(v[0])}/>, $el[0])
                    ...
                    $(v).each(function () {
                        ...
                        renderRbcomp(<UserShow id={$v[0]} name={$v[1]}
                                               onClick={() => _this.clickViewUser($v[0])}/>, item[0])
                    })

                    if (this.__ep && this.__ep.S === true) {
                        const item_op = $('<li class="list-inline-item"></li>').appendTo(list)[0]
                        if (v.length === 0) renderRbcomp(<UserShow name="添加共享" icon="zmdi zmdi-plus" onClick={() => {
                            $('.J_share').trigger('click')
                        }}/>, item_op)
                        else renderRbcomp(<UserShow name="管理共享用户" icon="zmdi zmdi-more"
                                                    onClick={() => DlgShareManager.create(this.__id)}/>, item_op)
                    } else if (v.length > 0) {
                        const item_op = $('<li class="list-inline-item"></li>').appendTo(list)[0]
                        renderRbcomp(<UserShow name="查看共享用户" icon="zmdi zmdi-more"
                                               onClick={() => DlgShareManager.create(this.__id, false)}/>, item_op)
                    } else {
    ...
    initVTabs(config) {
        ...
        $(config).each(function () {
            ...
            tabNav.find('a').click(function () {
                tabPane.find('.related-list').length === 0 && renderRbcomp(<MixRelatedList entity={entity}
                                                                                           master={that.__id}/>, tabPane)
            })
        })
        this.updateVTabs()
