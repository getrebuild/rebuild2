                ...
                <div className="form-group row">
                    ...
                    <div className="col-sm-7 pt-1 down-1">
                        <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                            <input className="custom-control-input" type="radio" name="scope"
                                   checked={this.state.scope === 'ALL'} value="ALL" onChange={this.handleChange}/>
                            <span className="custom-control-label">公共</span>
                        </label>
                        <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                            <input className="custom-control-input" type="radio" name="scope"
                                   checked={this.state.scope === 'SELF'} value="SELF" onChange={this.handleChange}/>
        ...
        return <RbModal title="上传文件" ref={(c) => this._dlg = c} disposeOnHide={true}>
            <div className="form" ref={(c) => this._dropArea = c}>
                <div className="form-group row">
                    ...
                    <div className="col-sm-7">
                        <select className="form-control form-control-sm" name="inFolder"
                                defaultValue={this.props.inFolder} onChange={this.handleChange}>
                            {__FolderData.map((item) => {
...
class FolderTree extends React.Component {
    ...
    _renderItem(item) {
        return <li key={`folder-${item.id}`} className={this.state.activeItem === item.id ? 'active' : ''}>
            <a data-id={item.id} onClick={() => this._clickItem(item)}
               href={`#!/Folder/${item.id}`}>{item.text}{item.private &&
            <i title="私有" className="icon zmdi zmdi-lock"/>}</a>
    ...
    _handleEdit(item) {
        ...
        renderRbcomp(<FolderEditDlg call={() => filesNav && filesNav.loadData()}
                                    id={item.id} name={item.text} scope={item.private ? 'SELF' : 'ALL'}
                                    parent={item.parent}/>)
    }
...
class FilesList2 extends FilesList {
    ...
    renderExtras(item) {
        return <React.Fragment>
            ...
            <span className="op">
        <a title="下载" onClick={(e) => $stopEvent(e)}
           href={`${rb.baseUrl}/filex/download/${item.filePath}?attname=${$fileCutName(item.filePath)}`}
           target="_blank"><i className="icon zmdi zmdi-download fs-15 down-2"></i></a>
                {rb.fileSharable && <a title="分享" onClick={(e) => this._share(item, e)}><span><i
                    className="icon zmdi zmdi-share fs-14 down-1"></i></span></a>}
      </span>
        </React.Fragment>
    }
...
$(document).ready(() => {
    ...
    $('.J_upload-file').click(() => renderRbcomp(<FileUploadDlg call={() => filesList && filesList.loadData()}
