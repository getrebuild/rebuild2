                ...
                <div className="form-group">
                    ...
                    <div>
                        {selectedRows > 0 && <label className="custom-control custom-control-sm custom-radio mb-2">
                            <input className="custom-control-input" name="dataRange" type="radio"
                                   checked={~~this.state.dataRange === 1} value="1" onChange={this.handleChange}/>
                        ...
                        <label className="custom-control custom-control-sm custom-radio mb-2">
                            <input className="custom-control-input" name="dataRange" type="radio"
                                   checked={~~this.state.dataRange === 2} value="2" onChange={this.handleChange}/>
                            <span className="custom-control-label">当前页的数据 ({pageRows}条)</span>
                        </label>
                        <label className="custom-control custom-control-sm custom-radio mb-2">
                            <input className="custom-control-input" name="dataRange" type="radio"
                                   checked={~~this.state.dataRange === 3} value="3" onChange={this.handleChange}/>
                        ...
                        <label className="custom-control custom-control-sm custom-radio mb-1">
                            <input className="custom-control-input" name="dataRange" type="radio"
                                   checked={~~this.state.dataRange === 10} value="10" onChange={this.handleChange}/>
                    ...
                    {(this.state.updateContents || []).map((item) => {
                        return <div key={`update-${item.field}`}>
                            <div className="row">
                                ...
                                <div className="col-6">
                                    {item.op !== 'NULL' &&
                                    <span className="badge badge-light">{item.text || item.value}</span>}
                                    <a className="del" onClick={() => this.removeItem(item.field)}><i
...
class BatchUpdateEditor extends React.Component {
    ...
    renderValue() {
        ...
        if (field.type === 'PICKLIST' || field.type === 'STATE' || field.type === 'MULTISELECT' || field.type === 'BOOL'
            ...
            return <select className="form-control form-control-sm" multiple={field.type === 'MULTISELECT'}
                           ref={(c) => this._value = c} key={fieldKey}>
            ...
            return <input className="form-control form-control-sm" placeholder="新值" ref={(c) => this._value = c}
                          key={fieldKey} maxLength="255"/>
        }
    }
    ...
    buildItem() {
        ...
        if (this._value.tagName === 'SELECT') {
            const texts = $(this._value).select2('data').map((o) => {
                return o.text
            })
