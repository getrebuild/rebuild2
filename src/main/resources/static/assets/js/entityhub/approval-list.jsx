        ...
        return <React.Fragment>
            {(this.state.data || []).map((item) => {
                return <tr key={'k-' + item[0]}>
                    ...
                    <td>{item[2] || item[1]}</td>
                    <td>{item[4] ? <span className="badge badge-warning font-weight-light">否</span> :
            ...
            {this.props.id && <div className="form-group row">
                <div className="col-sm-7 offset-sm-3">
                    <label className="custom-control custom-control-sm custom-checkbox custom-control-inline mb-0">
                        ...
                        <span className="custom-control-label">是否禁用 <i ref={(c) => this._tooltip = c}
                                                                       className="zmdi zmdi-help zicon"
                                                                       title="禁用后正在使用此流程的审批记录不受影响"></i></span>
                    </label>
                </div>
            </div>
            }
