...
class ContentFieldAggregation extends ActionContentSpec {
                            ...
                            <div className="col-5">
                                <select className="form-control form-control-sm" ref={(c) => this._targetEntity = c}>
                                    {(this.state.targetEntities || []).map((item) => {
                                        return <option key={'te-' + item[2] + item[0]}
                                                       value={item[2] + '.' + item[0]}>{item[1]}</option>
                                    })}
                                </select>
                            </div>
                        </div>
                        {this.state.hadApproval
                        && <div className="form-text text-danger"><i
                            className="zmdi zmdi-alert-triangle fs-16 down-1"></i> 目标实体已启用审批流程，可能影响源实体操作（触发动作）</div>}
                    </div>
                </div>
                <div className="form-group row">
                    ...
                    <div className="col-md-12 col-lg-9">
                                ...
                                return <div key={'item-' + item.targetField}>
                                    <div className="row">
                                        <div className="col-5"><span
                                            className="badge badge-warning">{this._getFieldLabel(this.state.targetFields, item.targetField)}</span>
                                        </div>
                                        <div className="col-2">
                      ...
                      <span className="badge badge-warning">
                        {item.calcMode === 'FORMULA' ? this.textFormula(item.sourceFormula) : this._getFieldLabel(this.state.sourceFields, item.sourceField)}
                                            ...
                                            <a className="del" title="移除"
                                               onClick={() => this.delItem(item.targetField)}><span
                                                className="zmdi zmdi-close"></span></a>
                                        </div>
                                    </div>
                                </div>
                            })}
                        ...
                        <div className="row">
                            ...
                            <div className="col-5">
                                <div className={this.state.calcMode === 'FORMULA' ? '' : 'hide'}>
                                    <div className="form-control-plaintext formula" ref={(c) => this._$formula = c}
                        ...
                        <div className="mt-1">
                            <button type="button" className="btn btn-primary btn-sm bordered"
                                    onClick={() => this.addItem()}>添加
                            </button>
                        </div>
                    </div>
                </div>
    ...
    showFormula = () => {
        renderRbcomp(<FormulaCalc fields={this.state.sourceFields}
                                  call={(v) => $(this._$formula).attr('data-v', v).text(this.textFormula(v))}/>)
    }
    ...
    _dataAdvFilter = () => {
        ...
        if (that._advFilter) that._advFilter.show()
                                     ...
                                     confirm={that._saveAdvFilter}/>, null, function () {
            ...
            <div className="row">
                    ...
                    <div className="fields rb-scroller" ref={(c) => this._$fields = c}>
                        <ul className="list-unstyled mb-0">
                            {this.props.fields.map((item) => {
                                return <li key={`flag-${item}`}><a onClick={() => this.handleInput(item)}>{item[1]}</a>
                                </li>
                            })}
                        </ul>
                    </div>
                ...
                <div className="col-6 pl-0">
                    <ul className="list-unstyled numbers mb-0">
                        {INPUT_KEYS.map((item) => {
                            return <li className="list-inline-item" key={`flag-${item}`}><a
                                onClick={() => this.handleInput(item)}>{item}</a></li>
                        })}
                        <li className="list-inline-item"><a onClick={() => this.confirm()} className="confirm">确定</a>
                        </li>
                    </ul>
                </div>
            </div>
