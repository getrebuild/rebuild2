...
$(document).ready(() => {
    ...
    $('.J_add-plan').click(() =>
        renderRbcomp(<PlanEdit projectId={wpc.id} flowNexts={_PlanList.getPlans()}
                               seq={_PlanList.getMaxSeq() + 1000}/>))
...
class PlanList extends React.Component {
                    ...
                    return (
                        <div className="card" key={`plan-${item[0]}`} data-id={item[0]} data-seq={item[4]}>
                            ...
                            <div className="card-footer card-footer-contrast">
                                ...
                                <a onClick={() => this._handleDelete(item[0])} className="danger"><i
    ...
    _handleEdit(item) {
        ...
        renderRbcomp(<PlanEdit id={item[0]} planName={item[1]} flowStatus={item[2]} flowNexts={otherPlans}
                               selectedFlowNexts={(item[3] || '').split(',')}/>)
    }
                ...
                <div className="form">
                    <div className="form-group row">
                        ...
                        <div className="col-sm-7">
                            <input type="text" className="form-control form-control-sm" name="planName"
                                   value={this.state.planName || ''} onChange={this.handleChange} maxLength="60"
                                   autoFocus/>
                        </div>
                    </div>
                    <div className="form-group row">
                        ...
                        <div className="col-sm-7">
                            <label className="custom-control custom-control-sm custom-radio mb-1 mt-1">
                                <input className="custom-control-input" type="radio" name="flowStatus" value="1"
                                       checked={~~this.state.flowStatus === 1} onChange={this.handleChange}/>
                                <span className="custom-control-label">开始状态 <p
                                    className="text-muted mb-0 fs-12">该状态下可新建任务</p></span>
                            </label>
                            <label className="custom-control custom-control-sm custom-radio mb-1">
                                <input className="custom-control-input" type="radio" name="flowStatus" value="2"
                                       checked={~~this.state.flowStatus === 2} onChange={this.handleChange}/>
                                <span className="custom-control-label">进行中 <p
                                    className="text-muted mb-0 fs-12">该状态下不可新建任务，不可完成任务</p></span>
                            </label>
                            <label className="custom-control custom-control-sm custom-radio mb-1">
                                <input className="custom-control-input" type="radio" name="flowStatus" value="3"
                                       checked={~~this.state.flowStatus === 3} onChange={this.handleChange}/>
                                <span className="custom-control-label">结束状态 <p
                                    className="text-muted mb-0 fs-12">该状态下任务自动标记完成</p></span>
                            </label>
                        </div>
                    </div>
                        ...
                        <div className="form-group row pt-0">
                            ...
                            <div className="col-sm-7 pt-1" ref={(c) => this._flowNexts = c}>
                                {this.props.flowNexts.map((item) => {
                                    return (
                                        <label
                                            className="custom-control custom-control-sm custom-checkbox custom-control-inline"
                                            key={`plan-${item[0]}`}>
                                            <input className="custom-control-input" type="checkbox" value={item[0]}
                                                   defaultChecked={selectedFlowNexts.includes(item[0])}/>
