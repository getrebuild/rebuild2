...
class RbFormModal extends React.Component {
                ...
                <div className="modal-dialog">
                    <div className="modal-content">
                        <div className="modal-header modal-header-colored">
                            ...
                            {rb.isAdminUser
                            && <a className="close s"
                                  href={rb.baseUrl + '/admin/entity/' + this.state.entity + '/form-design'} title="配置布局"
                                  target="_blank"><span className="zmdi zmdi-settings"></span></a>}
                            <button className="close md-close" type="button" onClick={() => this.hide()}><span
                                className="zmdi zmdi-close"></span></button>
                        </div>
                        <div className={'modal-body rb-loading' + (this.state.inLoad ? ' rb-loading-active' : '')}>
                            {this.state.alertMessage &&
                            <div className="alert alert-warning rbform-alert">{this.state.alertMessage}</div>}
    ...
    static create(props) {
        ...
        else renderRbcomp(<RbFormModal {...props} />, null, function () {
            that.__HOLDER = this
        })
    }
}
...
class RbForm extends React.Component {
    ...
    renderFormAction() {
        ...
        const moreActions = []
        if (pmodel.hadApproval) moreActions.push(<a key="Action103" className="dropdown-item"
                                                    onClick={() => this.post(103)}>保存并提交</a>)
        if (pmodel.isMaster === true) moreActions.push(<a key="Action102" className="dropdown-item"
                                                          onClick={() => this.post(102)}>保存并添加明细</a>)
        else if (pmodel.isSlave === true) moreActions.push(<a key="Action101" className="dropdown-item"
                                                              onClick={() => this.post(101)}>保存并继续添加</a>)

        let actionBtn = <button className="btn btn-primary btn-space" type="button"
                                onClick={() => this.post()}>保存</button>
        if (moreActions.length > 0) {
            actionBtn = <div className="btn-group dropup btn-space">
                ...
                <button className="btn btn-primary dropdown-toggle auto" type="button" data-toggle="dropdown"><span
                    className="icon zmdi zmdi-chevron-up"></span></button>
        ...
        return <div className="form-group row footer">
            <div className="col-12 col-sm-8 offset-sm-3" ref={(c) => this._formAction = c}>
                {actionBtn}
                <button className="btn btn-secondary btn-space" type="button"
                        onClick={() => this.props.$$$parent.hide()}>取消
                </button>
            </div>
        </div>
    }
...
class RbFormElement extends React.Component {
    ...
    render() {
        ...
        return <div className={`form-group row type-${props.type} ${editable ? 'editable' : ''}`}
                    data-field={props.field}>
            <label ref={(c) => this._fieldLabel = c}
                   className={`col-12 col-sm-${colWidths[0]} col-form-label text-sm-right ${(!props.onView && !props.nullable) ? 'required' : ''}`}>{props.label}</label>
            <div ref={(c) => this._fieldText = c} className={'col-12 col-sm-' + colWidths[1]}>
                ...
                <a className="edit" title="编辑" onClick={() => this.toggleEditMode(true)}/>}
                {editable && this.state.editMode && <div className="edit-oper">
                    <div className="btn-group shadow-sm">
                        <button type="button" className="btn btn-secondary" onClick={this.handleEditConfirm}><i
                            className="icon zmdi zmdi-check"></i></button>
                        <button type="button" className="btn btn-secondary" onClick={() => this.toggleEditMode(false)}>
                            <i className="icon zmdi zmdi-close"></i></button>
                    </div>
                </div>}
            </div>
        </div>
    ...
    renderElement() {
        ...
        return <input ref={(c) => this._fieldValue = c}
    ...
    onEditModeChanged(destroy) {
        if (destroy) {
            if (this.__select2) {
                if ($.type(this.__select2) === 'array') $(this.__select2).each(function () {
...
class RbFormUrl extends RbFormText {
    ...
    renderViewElement() {
        ...
        return (<div className="form-control-plaintext"><a href={clickUrl} className="link" target="_blank"
                                                           rel="noopener noreferrer">{this.state.value}</a></div>)
    }
...
class RbFormEMail extends RbFormText {
    ...
    renderViewElement() {
        if (!this.state.value) return super.renderViewElement()
        return <div className="form-control-plaintext"><a title="发送邮件" href={'mailto:' + this.state.value}
                                                          className="link">{this.state.value}</a></div>
    }
...
class RbFormPhone extends RbFormText {
    ...
    renderViewElement() {
        if (!this.state.value) return super.renderViewElement()
        return <div className="form-control-plaintext"><a title="拨打电话" href={'tel:' + this.state.value}
                                                          className="link">{this.state.value}</a></div>
    }
...
class RbFormTextarea extends RbFormElement {
    ...
    renderElement() {
        return <textarea ref={(c) => this._fieldValue = c}
...
class RbFormDateTime extends RbFormElement {
    ...
    renderElement() {
        ...
        return <div className="input-group datetime-field">
            <input ref={(c) => this._fieldValue = c}
                   className={'form-control form-control-sm ' + (this.state.hasError ? 'is-invalid' : '')}
                   title={this.state.hasError} type="text" value={this.state.value || ''} onChange={this.handleChange}
                   onBlur={this.checkValue}/>
            <span className={'zmdi zmdi-close clean ' + (this.state.value ? '' : 'hide')}
                  onClick={this.handleClear}></span>
            <div className="input-group-append">
                <button className="btn btn-secondary" type="button" ref={(c) => this._fieldValue__icon = c}><i
                    className="icon zmdi zmdi-calendar"></i></button>
            </div>
        </div>
    }
...
class RbFormImage extends RbFormElement {
    ...
    renderElement() {
        ...
        <input ref={(c) => this._fieldValue__input = c} type="file" className="inputfile"
               id={`${this.props.field}-input`} accept="image/*" data-maxsize="10240000"/>
        <label htmlFor={`${this.props.field}-input`} className="img-thumbnail img-upload"><span
            className="zmdi zmdi-image-alt"></span></label>
      </span>
            }
    ...
    renderViewElement() {
        ...
        return <div className="img-field">
            {value.map((item, idx) => {
                return <span key={'img-' + item}>
          <a title={$fileCutName(item)} onClick={() => (parent || window).RbPreview.create(value, idx)}
    ...
    renderElement() {
        ...
        return <div className="file-field">
            ...
            {showUpload && <div className="file-select">
                <input type="file" className="inputfile" ref={(c) => this._fieldValue__input = c}
                       id={`${this.props.field}-input`} data-maxsize="102400000"/>
                <label htmlFor={`${this.props.field}-input`} title={`上传文件。需要 ${this.__minUpload}~${this.__maxUpload} 个`}
        ...
        return <div className="file-field">
            {value.map((item) => {
                ...
                return <a key={'file-' + item} title={fileName}
                          onClick={() => (parent || window).RbPreview.create(item)} className="img-thumbnail">
    ...
    renderElement() {
        ...
        return (
            <div className="input-group datetime-field">
                ...
                <div className="input-group-append">
                    <button className="btn btn-secondary" type="button" onClick={this.showSearch}><i
                        className="icon zmdi zmdi-search"/></button>
                </div>
            </div>
        )
    }
...
class RbFormClassification extends RbFormElement {
    ...
    renderElement() {
        ...
        return <div className="input-group datetime-field">
            <select ref={(c) => this._fieldValue = c} className="form-control form-control-sm"/>
            <div className="input-group-append">
                <button className="btn btn-secondary" type="button" onClick={this.showSelector}><i
                    className="icon zmdi zmdi-search"/></button>
            </div>
        </div>
    }
    ...
    showSelector = () => {
        if (this.__selector) this.__selector.show()
            ...
            const p = this.props
            const that = this
            renderRbcomp(<ClassificationSelector entity={p.$$$parent.state.entity} field={p.field} label={p.label}
                                                 openLevel={p.openLevel} $$$parent={this}/>,
                null, function () {
...
class RbFormMultiSelect extends RbFormElement {
        ...
        return <div className="mt-1" ref={(c) => this._fieldValue__wrap = c}>
            ...
            {(this.props.options || []).map((item) => {
                return <label key={name + item.mask} className="custom-control custom-checkbox custom-control-inline">
                    <input className="custom-control-input" name={name} type="checkbox"
                           checked={(this.state.value & item.mask) !== 0} value={item.mask}
    ...
    changeValue = () => {
        let maskValue = 0
        $(this._fieldValue__wrap).find('input:checked').each(function () {
            maskValue += ~~$(this).val()
        })
    ...
    renderElement() {
        return <div className="mt-1">
            <label className="custom-control custom-radio custom-control-inline">
                <input className="custom-control-input" name={'radio-' + this.props.field} type="radio"
                       checked={this.state.value === 'T'} data-value="T"
                       onChange={this.changeValue} disabled={this.props.readonly}/>
                <span className="custom-control-label">是</span>
            </label>
            <label className="custom-control custom-radio custom-control-inline">
                <input className="custom-control-input" name={'radio-' + this.props.field} type="radio"
                       checked={this.state.value === 'F'} data-value="F"
                       onChange={this.changeValue} disabled={this.props.readonly}/>
...
class RbFormBarcode extends RbFormElement {
    ...
    renderElement() {
        if (this.state.value) return this.renderViewElement()
            ...
            className="form-control-plaintext barcode text-muted">{`自动值 (${this.props.barcodeType === 'QRCODE' ? '二维码' : '条形码'})`}</div>
    }

    renderViewElement() {
        if (!this.state.value) return super.renderViewElement()
        const codeUrl = `${rb.baseUrl}/commons/barcode/render${this.props.barcodeType === 'QRCODE' ? '-qr' : ''}?t=${$encode(this.state.value)}`
        return <div className="img-field barcode"><a className="img-thumbnail" title={this.state.value}><img
            src={codeUrl} alt={this.state.value}/></a></div>
    }
}
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <div className="text-center ml-6 mr-6">
                            <div className="text-danger"><span className="modal-main-icon zmdi zmdi-alert-triangle"/>
                            </div>
                            <div className="mt-3 text-bold">{message}</div>
                            {!this.props.entity ? null :
                                <div className="mt-2">
                                    <label
                                        className="custom-control custom-control-sm custom-checkbox custom-control-inline mb-2">
                                    ...
                                    <div className={' ' + (this.state.enableCascade ? '' : 'hide')}>
                                        <select className="form-control form-control-sm" ref={(c) => this._cascades = c}
                                            ...
                                            {(this.state.cascadesEntity || []).map((item) => {
                                                return <option key={'option-' + item[0]}
                            ...
                            <div className="mt-4 mb-3" ref={(c) => this._btns = c}>
                                <button className="btn btn-space btn-secondary" type="button"
                                        onClick={() => this.hide()}>取消
                                </button>
                                <button className="btn btn-space btn-danger" type="button"
                                        onClick={() => this.handleDelete()}>删除
                                </button>
                            </div>
                        </div>
                    </div>
...
class RepeatedViewer extends RbModalHandler {
    ...
    render() {
        ...
        return <RbModal ref={(c) => this._dlg = c} title={`存在 ${this.props.data.length - 1} 条重复记录`} disposeOnHide={true}
                        colored="warning">
    ...
    renderLine(item, idx) {
        return <tr key={`row-${idx}`}>
            ...
            <td className="actions"><a className="icon" onClick={() => this.openView(item[0])} title="查看详情"><i
                className="zmdi zmdi-open-in-new"/></a></td>
        </tr>
    }
