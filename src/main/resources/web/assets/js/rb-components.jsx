...
class RbModal extends React.Component {
    ...
    render() {
        ...
        return <div className={`modal rbmodal colored-header colored-header-${this.props.colored || 'primary'}`}
                    ref={(c) => this._rbmodal = c}>
            <div className="modal-dialog" style={{maxWidth: `${this.props.width || 680}px`}}>
                <div className="modal-content">
                    ...
                    <div
                        className={`modal-body ${inFrame ? 'iframe rb-loading' : ''} ${inFrame && this.state.frameLoad !== false ? 'rb-loading-active' : ''}`}>
                        {this.props.children ||
                        <iframe src={this.props.url} frameBorder="0" scrolling="no" onLoad={() => this.resize()}/>}
                        {inFrame && <RbSpinner/>}
                    </div>
                </div>
            </div>
    ...
    componentDidMount() {
        const $root = $(this._rbmodal).modal({
            show: true,
    ...
    static create(url, title, ext) {
        ...
        if (ext.disposeOnHide === false && !!that.__HOLDERs[url]) {
            ...
            renderRbcomp(<RbModal url={url} title={title} width={ext.width}
                                  disposeOnHide={ext.disposeOnHide}/>, null, function () {
                that.__HOLDER = this
        ...
        return (
            <div className="text-center ml-6 mr-6">
                ...
                <div className="mt-4 mb-3">
                    <button disabled={this.state.disable} className="btn btn-space btn-secondary" type="button"
                            onClick={cancel}>{this.props.cancelText || '取消'}</button>
                    <button disabled={this.state.disable} className={`btn btn-space btn-${type}`} type="button"
                            onClick={confirm}>{this.props.confirmText || '确定'}</button>
                </div>
            </div>
        )
    }
        ...
        return (
            <div ref={(c) => this._rbhighbar = c} className={`rbhighbar animated faster ${this.state.animatedClass}`}>
                <div className={`alert alert-dismissible alert-${(this.props.type || 'warning')} mb-0`}>
                    <button className="close" type="button" onClick={this.close}><i className="zmdi zmdi-close"/>
                    </button>
        ...
        <div className={`alert alert-icon alert-icon-border alert-dismissible alert-sm alert-${type}`}>
            ...
            <div className="message">
                <a className="close" data-dismiss="alert"
                   onClick={() => typeof props.onClose === 'function' && props.onClose()}><i
        ...
        const _DropdownMenu = (
            <div className="dropdown-menu">
                <div className="selector-search">
                    <div>
                        <input type="search" className="form-control search" placeholder="输入关键词搜索"
                               value={this.state.query || ''} ref={(c) => this._input = c}
                        ...
                        {this._useTabs.map((item) => {
                            return (
                                <li className="nav-item" key={`t-${item[0]}`}>
                                    <a onClick={() => this.switchTab(item[0])}
                                       className={`nav-link ${this.state.tabType === item[0] ? ' active' : ''}`}>{item[1]}</a>
                                </li>
                            )
                        })}
                    </ul>
                            ...
                            <div className="rb-scroller" ref={(c) => this._scroller = c}>
                                <ul className="select2-results__options">
                                    {inResult ? inResult : this.state.items.map((item) => {
                                        return (
                                            <li key={`o-${item.id}`} className="select2-results__option"
                                                data-id={item.id} onClick={(e) => this.clickItem(e)}>
                                                <i className={`zmdi ${!this.props.hideSelection && this.containsItem(item.id) ? ' zmdi-check' : ''}`}></i>
                                                {this.state.tabType === 'User' &&
          ...
          {this.props.compToggle ?
              ...
              <div className="select2-selection select2-selection--multiple">
                  <div className="select2-selection__rendered" ref={(c) => this._dropdownParent = c}>
                      ...
                      <span className="select2-selection__clear" onClick={() => this.clearSelection()}>&times;</span>}
                      {this.state.selected.map((item) => {
                          return (
                              <span key={`s-${item.id}`} className="select2-selection__choice">
                      <span className="select2-selection__choice__remove" data-id={item.id}
                            onClick={(e) => this.removeItem(e)}>&times;</span> {item.text}
                    </span>
                          )
                      })}
...
const UserShow = function (props) {
    ...
    return (
        <a href={viewUrl} className="user-show" title={props.name} onClick={props.onClick}>
            <div className={`avatar ${props.showName === true ? ' float-left' : ''}`}>{props.icon ?
                <i className={props.icon}/> : <img src={avatarUrl} alt="Avatar"/>}</div>
            {props.showName && (
                <div className={`text-truncate name ${props.deptName ? 'vm' : ''}`}>{props.name}{props.deptName &&
                <em>{props.deptName}</em>}</div>)}
        </a>
    )
}
