            ...
            <div className={`preview-modal ${this.state.inLoad ? 'hide' : ''}`} ref={(c) => this._dlg = c}>
                <div className="preview-header">
                    ...
                    <div className="float-right">
                        ...
                        <a title="下载" target="_blank" rel="noopener noreferrer" href={downloadUrl}><i
                            className="zmdi zmdi-download"></i></a>
                        {!this.props.unclose &&
                        <a title="关闭 (ESC)" onClick={this.hide}><i className="zmdi zmdi-close"></i></a>}
                    </div>
                    <div className="clearfix"></div>
                </div>
                <div className="preview-body" onClick={HIDE_ONCLICK ? this.hide : () => { /*NOOP*/
                }} ref={(c) => this._previewBody = c}>
                    {previewContent}
                </div>
            </div>
        </React.Fragment>
                ...
                <div className="expires mt-2">
                    <ul className="list-unstyled">
                        {EXPIRES_TIME.map((item) => {
                            return <li key={`time-${item[0]}`}
                                       className={`list-inline-item ${this.state.time === item[0] && 'active'}`}>
