...
class ContentSendNotification extends ActionContentSpec {
        ...
        return <div className="send-notification">
            <form className="simple">
                <div className="form-group row pt-1">
                    ...
                    <div className="col-12 col-lg-8 pt-1">
                        <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                            <input className="custom-control-input" name="mtype" type="radio"
                                   onChange={() => this.setState({type: 1})} checked={this.state.type === 1}/>
                            <span className="custom-control-label">消息通知</span>
                        </label>
                        <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                            <input className="custom-control-input" name="mtype" type="radio"
                                   onChange={() => this.setState({type: 2})} checked={this.state.type === 2}/>
                            <span
                                className="custom-control-label">邮件 {this.state.serviceMail === false && '(不可用)'}</span>
                        </label>
                        <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-1">
                            <input className="custom-control-input" name="mtype" type="radio"
                                   onChange={() => this.setState({type: 3})} checked={this.state.type === 3}/>
                            <span
                                className="custom-control-label">短信 {this.state.serviceSms === false && '(不可用)'}</span>
                        </label>
                    </div>
                </div>
                <div className="form-group row pt-1">
                ...
                {this.state.type === 2 && <div className="form-group row pb-1">
                    ...
                    <div className="col-12 col-lg-8">
                        <input type="text" className="form-control form-control-sm" ref={(c) => this._title = c}
                               maxLength="60" placeholder="你有一条新通知"/>
                    </div>
                </div>
                }
                <div className="form-group row pb-1">
                    ...
                    <div className="col-12 col-lg-8">
                        <textarea className="form-control form-control-sm row3x" ref={(c) => this._content = c}
                                  maxLength="600"></textarea>
                        {rb.env === 'dev' &&
                        <p className="form-text">内容支持变量，例如 <code>{'{createdOn}'}</code>（其中 createdOn 为触发实体的字段内部标识）</p>}
                    </div>
                </div>
            </form>
        </div>
    ...
    buildContent() {
        ...
        if (!_data.sendTo || _data.sendTo.length === 0) {
            RbHighbar.create('请选择发送给谁');
            return false
        }
        if (!_data.content) {
            RbHighbar.create('内容不能为空');
            return false
        }
        return _data
    }
