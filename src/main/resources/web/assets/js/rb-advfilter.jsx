    ...
    render() {
        const opButtons = this.props.fromList ?
            <div className="float-right">
                ...
                <button className="btn btn-primary bordered" type="button" onClick={() => this.searchNow()}><i
                    className="icon zmdi zmdi-search"/> 立即查询
                </button>
            </div>
            :
            <div className="item">
            ...
            <div className={'adv-filter-wrap ' + (this.props.inModal ? 'in-modal' : 'shadow rounded')}>
                <div className="adv-filter">
                    <div className="filter-items" onKeyPress={this.searchByKey}>
                        ...
                        <div className="item plus"><a onClick={() => this.addItem()} tabIndex="-1"><i
                            className="zmdi zmdi-plus-circle icon"></i> 添加条件</a></div>
                    </div>
                </div>
                    ...
                    <div className="mb-1">
                        <div className="item mt-1">
                            <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-2">
                                <input className="custom-control-input" type="radio" name="useEquation" value="OR"
                                       checked={this.state.useEquation === 'OR'} onChange={this.handleChange}/>
                                <span className="custom-control-label"> 或关系</span>
                            </label>
                            <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-2">
                                <input className="custom-control-input" type="radio" name="useEquation" value="AND"
                                       checked={this.state.useEquation === 'AND'} onChange={this.handleChange}/>
                                <span className="custom-control-label"> 且关系</span>
                            </label>
                            <label className="custom-control custom-control-sm custom-radio custom-control-inline mb-2">
                                <input className="custom-control-input" type="radio" name="useEquation" value="9999"
                                       checked={this.state.useEquation === '9999'} onChange={this.handleChange}/>
                        ...
                        <div className="mb-3 equation-state">
                            <input
                                ...
                                title={(this.state.equationError ? '高级表达式有误' : '')} value={this.state.equation || ''}
                                placeholder={this.state.equationDef || ''} data-id="equation"
                                onChange={this.handleChange} onBlur={(e) => this.checkEquation(e)}/>
                            {this.state.equationError ? <i className="zmdi zmdi-alert-triangle text-danger"></i> :
                        ...
                        <div className="item dialog-footer">
                            <div className="float-left">
                                <div className="float-left input">
                                    <input className="form-control form-control-sm text" maxLength="20"
                                           value={this.state.filterName || ''} data-id="filterName"
                                           onChange={this.handleChange} placeholder="输入名称保存"/>
                                </div>
                                {rb.isAdminUser &&
        ...
        if (this.props.inModal) return <RbModal ref={(c) => this._dlg = c} title={this.props.title || '高级查询'}
                                                disposeOnHide={!!this.props.filterName}>{advFilter}</RbModal>
...
class FilterItem extends React.Component {
        ...
        return (
            <div className="row item">
                <div className="col-sm-5 field">
                    ...
                    <i className="zmdi zmdi-minus-circle" title="移除条件"
                       onClick={() => this.props.$$$parent.removeItem(this.props.id)}></i>
                    <select className="form-control form-control-sm" ref={(c) => this._filterField = c}>
                        {this.state.fields.map((item) => {
                            return <option value={item.name + NT_SPLIT + item.type} key={`field-${item.name}`}
                                           title={item.label}>{item.label}</option>
                        })}
                    </select>
                </div>
                <div className="col-sm-2 op">
                    <select className="form-control form-control-sm" ref={(c) => this._filterOp = c}>
                        {this.selectOp().map((item) => {
                            return <option value={item} key={`op-${item}`}
                                           title={OP_TYPE[item]}>{OP_TYPE[item]}</option>
                        })}
                    </select>
                </div>
    ...
    renderValue() {
        let val = <input className="form-control form-control-sm" ref={(c) => this._filterVal = c}
                         onChange={this.valueHandle} onBlur={this.valueCheck} value={this.state.value || ''}/>
        if (this.state.op === 'BW') {
            val = (
                <div className="val-range">
                    <input className="form-control form-control-sm" ref={(c) => this._filterVal = c}
                           onChange={this.valueHandle} onBlur={this.valueCheck} value={this.state.value || ''}/>
                    <input className="form-control form-control-sm" ref={(c) => this._filterVal2 = c}
                           onChange={this.valueHandle} onBlur={this.valueCheck} value={this.state.value2 || ''}
                           data-at="2"/>
                    <span>起</span>
    ...
    renderPickListAfter() {
        const that = this
        const s2val = $(this._filterVal).select2({}).on('change.select2', function () {
