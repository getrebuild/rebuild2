...
$(document).ready(function () {
    ...
    $('.J_imports').click(() => {
        ...
        else renderRbcomp(<DlgImports id={wpc.id}/>, null, function () {
            _dlgImports = this
        })
    })
            ...
            <div className={`col-md-3 ${this.state.turnOn ? '' : 'off'}`}>
                ...
                <form className="mt-1" onSubmit={this.saveItem}>
                    <div className="input-group input-group-sm">
                        <input className="form-control" type="text" maxLength="60" placeholder="名称"
                               value={this.state.itemName || ''} data-id="itemName" onChange={this.changeVal}/>
                        {this.state.itemId && this.state.itemHide &&
                        (
                            <label className="custom-control custom-control-sm custom-checkbox custom-control-inline">
                                <input className="custom-control-input" type="checkbox" data-id="itemUnhide"
                        ...
                        <div className="input-group-append">
                            <button className="btn btn-primary" type="submit"
                                    disabled={this.state.inSave === true}>{this.state.itemId ? '保存' : '添加'}</button>
                        </div>
                    </div>
                </form>
                <div className="rb-scroller mt-3">
                    <ol className="dd-list unset-list">
                        {(this.state.items || []).map((item) => {
                            ...
                            return (
                                <li className={`dd-item ${active ? ' active' : ''}`} key={item[0]}
                                    onClick={() => this.clickItem(item[0])}>
                                    <div className={`dd-handle ${item[3] ? ' text-disabled' : ''}`}>{item[1]}{item[3] &&
                                    <small/>}</div>
                                    <div className="dd-action">
                                        <a><i className="zmdi zmdi-edit" title="编辑"
                                              onClick={(e) => this.editItem(item, e)}></i></a>
                                        <a><i className="zmdi zmdi-delete" title="删除"
                                              onClick={(e) => this.delItem(item, e)}></i></a>
                                    </div>
            ...
            <RbModal title="导入分类数据" ref={(c) => this._dlg = c}>
                <div className="tab-container">
                    <ul className="nav nav-tabs">
                        <li className="nav-item"><a className="nav-link active" href="#FILE" data-toggle="tab">文件导入</a>
                        </li>
                        <li className="nav-item"><a className="nav-link" href="#RBSTORE" data-toggle="tab"><i
                            className="icon zmdi zmdi-cloud-outline-alt"></i> 从 RB 仓库导入</a></li>
                    </ul>
                    <div className="tab-content m-0 pb-0">
                                    ...
                                    <div className="col-sm-7">
                                        <div className="float-left">
                                            <div className="file-select">
                                                <input type="file" className="inputfile" id="upload-input"
                                                       accept=".xlsx,.xls,.csv" data-maxsize="20971520" data-temp="true"
                                                       ref={(c) => this._uploadInput = c}/>
                                                <label htmlFor="upload-input" className="btn-secondary"><i
                                                    className="zmdi zmdi-upload"></i><span>选择文件</span></label>
                                            </div>
                                        </div>
                                        ...
                                        <div className="form-text">
                                            支持 Excel 或 CSV 文件，文件格式请 <a
                                            href="https://getrebuild.com/docs/admin/classifications" target="_blank"
                                            className="link">参考文档</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="form-group row footer">
                                    <div className="col-sm-7 offset-sm-3">
                                        <button className="btn btn-primary" type="button"
                                                onClick={() => this.import4File()} disabled={this.state.inProgress}>开始导入
                                        </button>
                                        <button className="btn btn-link" type="button" onClick={() => this._dlg.hide()}
                                                disabled={this.state.inProgress}>取消
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="tab-pane" id="RBSTORE">
                                    ...
                                    return <div key={'data-' + item.file}>
                                        <div className="float-left">
                                            ...
                                            <div className="text-muted">
                                                数据来源 <a target="_blank" className="link" rel="noopener noreferrer"
                                                        href={item.source}>{item.author || item.source}</a>
                                                {item.updated && (' · ' + item.updated)}
                                            </div>
                                        </div>
                                        <div className="float-right">
                                            <button disabled={this.state.inProgress === true}
                                                    className="btn btn-sm btn-primary" data-file={item.file}
                                                    data-name={item.name} onClick={this.import4Rbstore}>导入
                                            </button>
                                        </div>
                                        <div className="clearfix"></div>
                                    </div>
                            ...
                            <div className="mt-2 mr-2 text-right">
                                <a href="https://github.com/getrebuild/rebuild-datas/" className="link" target="_blank"
                                   rel="noopener noreferrer">提交数据到 RB 仓库</a>
                            </div>
                        </div>
                    </div>
                </div>
