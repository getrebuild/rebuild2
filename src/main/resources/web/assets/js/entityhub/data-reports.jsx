        ...
        return <React.Fragment>
            {(this.state.data || []).map((item) => {
                return <tr key={'k-' + item[0]}>
                    <td>{item[3]}</td>
                    <td>{item[2] || item[1]}</td>
                    <td>{item[4] ? <span className="badge badge-warning font-weight-normal">否</span> :
                        <span className="badge badge-success font-weight-light">是</span>}</td>
                    <td>{item[5]}</td>
                    <td className="actions">
                        <a className="icon" title="预览"
                           href={`${rb.baseUrl}/admin/datas/data-reports/preview?id=${item[0]}`} target="_blank"><i
                            className="zmdi zmdi-eye"/></a>
                        <a className="icon" title="修改" onClick={() => this.handleEdit(item)}><i
                            className="zmdi zmdi-edit"/></a>
                        <a className="icon" title="删除" onClick={() => this.handleDelete(item[0])}><i
                    ...
                    <div className="col-sm-9">
                        <div className="float-left">
                            <div className="file-select">
                                <input type="file" className="inputfile" id="upload-input" accept=".xlsx,.xls"
                                       data-maxsize="5000000" ref={(c) => this.__upload = c}/>
                        ...
                        <p className="form-text mt-0 mb-1">如何编写模板文件？<a
                            href="https://getrebuild.com/docs/admin/excel-admin" target="_blank"
                            className="link">查看帮助</a></p>
