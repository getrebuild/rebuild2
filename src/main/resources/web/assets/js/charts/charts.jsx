    ...
    render() {
        const opers = (
            <div className="chart-oper">
                {!this.props.builtin &&
                <a title="查看来源数据" href={`${rb.baseUrl}/dashboard/view-chart-sources?id=${this.props.id}`}><i
                    className="zmdi zmdi-rss"/></a>}
                <a onClick={() => this.loadChartData()}><i className="zmdi zmdi-refresh"/></a>
                <a onClick={() => this.toggleFullscreen()}><i
                    className={`zmdi zmdi-${this.state.fullscreen ? 'fullscreen-exit' : 'fullscreen'}`}/></a>
                {this.props.editable && (
                    <React.Fragment>
                        {!this.props.builtin &&
                        <a className="chart-edit" href={`${rb.baseUrl}/dashboard/chart-design?id=${this.props.id}`}><i
                            className="zmdi zmdi-edit"/></a>}
        ...
        return (
            <div className={`chart-box ${this.props.type}`} ref={(c) => this._box = c}>
                ...
                <div ref={(c) => this._body = c}
                     className={'chart-body rb-loading ' + (!this.state.chartdata && 'rb-loading-active')}>{this.state.chartdata ||
                <RbSpinner/>}</div>
            </div>
        )
    }
...
class ApprovalList extends BaseChart {
    ...
    renderChart(data) {
        ...
        const table = (!data.data || data.data.length === 0) ?
            <div className="chart-undata must-center"><i className="zmdi zmdi-check icon text-success"></i> 你已完成所有审批
            </div>
            :
            <div>
                    ...
                    {data.data.map((item, idx) => {
                        return <tr key={'approval-' + idx}>
                            ...
                            <td className="actions text-right">
                                {this.state.viewState === 1 && <button className="btn btn-secondary btn-sm"
                                                                       onClick={() => this.approve(item[3], item[5], item[7])}>审批</button>}
    ...
    approve(record, approval, entity) {
        ...
        if (this.__approvalForms[record]) this.__approvalForms[record].show()
            ...
            renderRbcomp(<ApprovalApproveForm id={record} approval={approval} entity={entity}
                                              call={close}/>, null, function () {
                ...
                <table className="table table-striped table-hover">
                    ...
                    {data.map((item, idx) => {
                        ...
                        return <tr key={'schedule-' + idx}>
                            <td>
                                <a title="查看详情" href={`${rb.baseUrl}/app/list-and-view?id=${item.id}`}
                                   className="content" dangerouslySetInnerHTML={{__html: item.content}}/>
                            </td>
                            <td className="cell-detail">
                                <div>{item.scheduleTime}</div>
                                <span
                                    className={`cell-detail-description ${timeover ? 'text-warning' : ''}`}>{item.scheduleLeft}{timeover ? ' (过期)' : ''}</span>
                            </td>
                            <td className="actions text-right">
                                <button className="btn btn-secondary btn-sm"
                                        onClick={() => this.handleFinish(item.id)}>完成
                                </button>
                            </td>
                        </tr>
                    })}
            ...
            <div className="row chart-select-wrap">
                <div className="col-3">
                    <div className="nav flex-column nav-pills">
                        <a href="#all" onClick={this.switchTab}
                           className={`nav-link ${this.state.tabActive === '#all' ? 'active' : ''}`}>全部</a>
                        {this.props.entity && <a href="#entity" onClick={this.switchTab}
                                                 className={`nav-link ${this.state.tabActive === '#entity' ? 'active' : ''}`}>当前实体</a>}
                        <a href="#myself" onClick={this.switchTab}
                           className={`nav-link hide ${this.state.tabActive === '#myself' ? 'active' : ''}`}>我自己的</a>
                        <a href="#builtin" onClick={this.switchTab}
                           className={`nav-link ${this.state.tabActive === '#builtin' ? 'active' : ''}`}>内置图表</a>
                    </div>
                </div>
                <div className="col-9 pl-0">
                    <div className="chart-list">
                        {(this.state.chartList && this.state.chartList.length === 0) &&
                        <p className="text-muted">无可用图表</p>}
                  ...
                  {this.state.appended.includes(item[0])
                      ...
                      : <a className="btn" onClick={() => this.selectChart(item)}>添加</a>}
                                ...
                                {(!this.props.entity && item[4]) && <span className="float-right"><a className="delete"
                                                                                                     onClick={() => this.deleteChart(item[0])}><i
                                    className="zmdi zmdi-delete"></i></a></span>}
