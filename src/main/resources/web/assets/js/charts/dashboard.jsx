...
const dlgShow = (t, props) => {
    ...
    if (dlgRefs[t]) dlgRefs[t].show()
        ...
        if (dash_editable) renderRbcomp(<DlgAddChart {...props} />, null, function () {
            dlgRefs[t] = this
        })
        else RbHighbar.create('你无权添加图标到此仪表盘')
    } else if (t === 'DlgDashAdd') renderRbcomp(<DlgDashAdd {...props} />, null, function () {
        dlgRefs[t] = this
    })
    else if (t === 'DlgDashSettings') renderRbcomp(<DlgDashSettings {...props} />, null, function () {
        dlgRefs[t] = this
    })
    else if (t === 'DashSelect') renderRbcomp(<DashSelect {...props} />, null, function () {
        dlgRefs[t] = this
    })
}
            ...
            <RbModal title="仪表盘设置" ref="dlg">
                <div className="form">
                    <div className="form-group row">
                        ...
                        <div className="col-sm-7">
                            <input className="form-control form-control-sm" value={this.state.title || ''}
                                   placeholder="默认仪表盘" data-id="title" onChange={this.handleChange} maxLength="40"/>
                        </div>
                    </div>
                    ...
                    <div className="form-group row footer">
                        <div className="col-sm-7 offset-sm-3">
                            <button className="btn btn-primary btn-space" type="button" onClick={() => this.save()}>确定
                            </button>
                            <button className="btn btn-danger bordered btn-space" type="button"
                                    onClick={() => this.delete()}><i className="zmdi zmdi-delete icon"/> 删除
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <RbModal title="添加仪表盘" ref="dlg">
                <div className="form">
                    <div className="form-group row">
                        ...
                        <div className="col-sm-7">
                            <input className="form-control form-control-sm" value={this.state.title || ''}
                                   placeholder="我的仪表盘" data-id="title" onChange={this.handleChange} maxLength="40"/>
                        </div>
                    </div>
                    <div className="form-group row">
                        <label className="col-sm-3 col-form-label text-sm-right"></label>
                        <div className="col-sm-7">
                            <label
                                ...
                                <input className="custom-control-input" type="checkbox"
                                       checked={this.state.copy === true} data-id="copy" onChange={this.handleChange}/>
                            ...
                            <div>
                                <ul className="list-unstyled">
                                    {(this.props.dashList || []).map((item) => {
                                        return <li key={'dash-' + item[0]}><a href={'?d=' + item[0]}>{item[4]}<i
                                            className="icon zmdi zmdi-arrow-right"></i></a></li>
                                    })}
                                </ul>
                            </div>
                        </div>
