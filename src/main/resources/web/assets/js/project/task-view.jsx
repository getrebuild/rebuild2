            ...
            <div className="rbview-form task-form">
                <div className="form-group row pt-0">
                    ...
                    <div className="col-2 text-right">
                        <button className="btn btn-secondary" style={{minWidth: 80, marginTop: 2}}
                                data-toggle="dropdown">操作 <i className="icon zmdi zmdi-more-vert"></i></button>
                ...
                <div className="form-group row">
                    <label className="col-12 col-sm-3 col-form-label"><i
                        className="icon zmdi zmdi-square-o"/> 状态</label>
                ...
                <div className="form-group row">
                    <label className="col-12 col-sm-3 col-form-label"><i
                        className="icon zmdi zmdi-account-o"/> 执行人</label>
                    <div className="col-12 col-sm-9">
                ...
                <div className="form-group row">
                    <label className="col-12 col-sm-3 col-form-label"><i
                        className="icon zmdi zmdi-comment-more"/> 备注</label>
                    <div className="col-12 col-sm-9">
                ...
                <div className="form-group row">
                    <label className="col-12 col-sm-3 col-form-label"><i
                        className="icon zmdi zmdi-circle-o"/> 优先级</label>
                    <div className="col-12 col-sm-9">
                ...
                <div className="form-group row">
                    <label className="col-12 col-sm-3 col-form-label"><i
                        className="icon zmdi zmdi-attachment-alt zmdi-hc-rotate-45 mt-1"/> 附件</label>
                    <div className="col-12 col-sm-9">
...
class ValueTaskName extends ValueComp {
    ...
    renderElement() {
        return this.props.$$$parent.props.editable && this.state.editMode ?
            <input type="text" className="task-title" name="taskName" defaultValue={this.state.taskName}
...
class ValueExecutor extends ValueComp {
                ...
                {this.state.executor ?
                    <div className="executor-show">
                        <UserSelector hideDepartment={true} hideRole={true} hideTeam={true} multiple={false}
                                      ...
                                      compToggle={<span data-toggle="dropdown" style={{
                                          height: 30,
                                          display: 'inline-block'
                                      }}>{this._renderValue()}</span>}
                    ...
                    <div className="form-control-plaintext">
                        <UserSelector hideDepartment={true} hideRole={true} hideTeam={true} multiple={false}
                                      ref={(c) => this._UserSelector = c}
                                      compToggle={<a className="tag-value arrow placeholder"
                                                     data-toggle="dropdown">选择执行人</a>}
                                      onSelectItem={(s) => this.handleChange(s)}
                        />
                    </div>
                }
    ...
    _renderValue(call) {
        return typeof this.state.executor === 'object' &&
            <UserShow id={this.state.executor[0]} name={this.state.executor[1]}
                      showName
    ...
    renderElement() {
        return (
            <div className="form-control-plaintext" ref={(c) => this._deadline = c}>
                <a className={`tag-value arrow ${this.state.deadline ? 'plaintext' : 'placeholder'}`} name="deadline"
                   title={this.state.deadline}>
...
class ValuePriority extends ValueComp {
    ...
    renderElement() {
        return (
            <div className="form-control-plaintext">
                <a className={`tag-value arrow priority-${this.state.priority}`}
                   data-toggle="dropdown">{__PRIORITIES[this.state.priority]}</a>
    ...
    renderViewElement() {
        return <div className="form-control-plaintext">
            <span className={`tag-value arrow priority-${this.state.priority}`}
                  data-toggle="dropdown">{__PRIORITIES[this.state.priority]}</span>
        </div>
    }
            ...
            <React.Fragment>
                <div className="form-control-plaintext">
                    <input type="file" className="inputfile" id="attachments" ref={(c) => this._attachments = c}
                           data-maxsize="102400000"/>
                    <label htmlFor="attachments" style={{padding: 0, border: 0, lineHeight: 1}}><a
                        className="tag-value upload hover">+ 上传</a></label>
                </div>
        ...
        return (this.state.attachments &&
            <div className="file-field attachments">
                {this.state.attachments.map((item) => {
                    ...
                    return (
                        <a key={`file-${item}`} className="img-thumbnail" title={fileName}
                           onClick={() => (parent || window).RbPreview.create([item])}>
                            <i className="file-icon" data-type={$fileExtName(fileName)}/><span>{fileName}</span>
                            {del && <b title="删除" onClick={(e) => this._deleteAttachment(item, e)}><span
                                className="zmdi zmdi-delete"></span></b>}
                        </a>
                    )
                })}
            </div>
                                        ...
                                        <div className="actions">
                                            ...
                                            <ul className="list-unstyled m-0">
                                                {item.self && <li className="list-inline-item mr-2">
                                                    <a href="#reply" onClick={() => this._handleDelete(item)}
                                                ...
                                                <li className="list-inline-item">
                                                    <a href="#reply" onClick={() => this._handleReply(item)}
                                                       className="fixed-icon">
        ...
        return (
            <div className="comments">
                <div className="comment-reply">
                    <div onClick={() => this.commentState(true)}
          ...
          <textarea ref={(c) => this._textarea = c} placeholder={this.props.placeholder} maxLength="2000"
                    ...
                    <div className="action-btns">
                        <ul className="list-unstyled list-inline m-0 p-0">
                            ...
                            <li className="list-inline-item">
                                <a title="附件" onClick={() => this._fileInput.click()}><i
                ...
                <span className="hide"><input type="file" ref={(c) => this._fileInput = c}
                                              data-maxsize="102400000"/></span>
            </React.Fragment>
        )
    }
