...
$(document).ready(() => {
    ...
    $('.J_filter').click(() => {
        ...
        else renderRbcomp(<AdvFilter title="高级查询" entity="ProjectTask" inModal={true} canNoFilters={true}
                                     confirm={onFilter}/>, null, function () {
            __AdvFilter = this
        })
    })
})
            ...
            <React.Fragment>
                {this.props.plans.map((item) => {
                    return <PlanBox key={`plan-${item.id}`} readonly={this.props.readonly}
                                    id={item.id} planName={item.planName} flowStatus={item.flowStatus}
                    ...
                    <textarea className="form-control form-control-sm row2x" placeholder="输入标题以新建任务"
                              ...
                              autoFocus/>
                                    ...
                                    <div>
                                        ...
                                        <div>
                                            <UserSelector hideDepartment={true} hideRole={true} hideTeam={true}
                                                          multiple={false} ref={(c) => this._executor = c}/>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div>
                                            <input type="text" className="form-control form-control-sm"
                                                   ref={(c) => this._deadline = c}/>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <button className="btn btn-link w-auto" type="button"
                                                onClick={() => this.setState({newMode: false})}>取消
                                        </button>
                                        <button className="btn btn-primary" type="button" ref={(c) => this._btn = c}
                                                onClick={() => this._handleCreateTask()}>确定
                                        </button>
                                    </div>
                                </div>
                            </div>
    ...
    render() {
        return (
            <div className={`task-card content status-${this.state.status} priority-${this.state.priority}`}
                 data-seq={this.state.seq} data-taskid={this.state.id} data-planid={this.props.planid}
                 onClick={() => TaskViewModal.create(this.state.id)}>
                <div className="task-card-body">
                    <div className="task-content-wrapper">
                        <div className="task-status">
                            <label className="custom-control custom-control-sm custom-checkbox custom-control-inline"
                                   title="已完成/未完成" onClick={(e) => $stopEvent(e)}>
                                <input className="custom-control-input" type="checkbox"
                                       defaultChecked={this.state.status === 1} onChange={(e) => this._toggleStatus(e)}
                        ...
                        <div className="task-content">
                            ...
                            {this.state.endTime &&
                            <div className="task-time">完成于 <DateShow date={this.state.endTime}/></div>}
                    ...
                    <div className="modal-content">
                        <div
                            ...
                            <iframe ref={(c) => this._iframe = c} className={this.state.isHide ? 'invisible' : ''}
                                    src={`${rb.baseUrl}/project/task/${this.state.taskid}`} frameBorder="0"
                                    scrolling="no"></iframe>
                            <RbSpinner/>
                        </div>
                    </div>
