...
class RbList extends React.Component {
    ...
    render() {
        ...
        return <React.Fragment>
                        ...
                        <table className="table table-hover table-striped">
                            ...
                            <tr>
                                    ...
                                    <div>
                                        <label className="custom-control custom-control-sm custom-checkbox">
                                            <input className="custom-control-input" type="checkbox"
                                                   onChange={(e) => this._toggleRows(e)}
                                                   ref={(c) => this._checkAll = c}/>
                                ...
                                {this.state.fields.map((item, idx) => {
                                    ...
                                    return <th key={'column-' + item.field} style={styles} className={clazz}
                            ...
                            {this.state.rowsData.map((item) => {
                                ...
                                return (
                                    <tr key={rowKey} data-id={lastPrimary.id} onClick={(e) => this._clickRow(e, true)}>
                                        {this.props.uncheckbox !== true &&
                                        <td key={rowKey + '-checkbox'}
                                            className={`column-checkbox ${supportFixedColumns ? 'column-fixed' : ''}`}>
                                            <div>
                                                <label className="custom-control custom-control-sm custom-checkbox">
                                                    <input className="custom-control-input" type="checkbox"
                                                           onChange={(e) => this._clickRow(e)}/>
                                                    <span className="custom-control-label"></span>
                                                </label>
                                            </div>
                                        </td>
            ...
            && <RbListPagination ref={(c) => this._pagination = c} rowsTotal={this.state.rowsTotal}
                                 pageSize={this.pageSize} $$$parent={this}/>}
        ...
        <td key={k}>
            <div style={s} title={v.text}>
                <a href={'#!/View/' + v.entity + '/' + v.id} onClick={(e) => CellRenders.clickView(v, e)}
                   className="column-main">{v.text}</a>
            </div>
        </td>
    )
})
        ...
        <td key={k} className="td-min">
            <div className="column-imgs" style={s} title={'共 ' + v.length + ' 个图片'}>
                {v.map((item, idx) => {
                    ...
                    return <a key={'k-' + item} title={imgName}
                              onClick={(e) => CellRenders.clickPreview(v, idx, e)}><img alt="图片"
                                                                                        src={imgUrl + '?imageView2/2/w/100/interlace/1/q/100'}/></a>
                })}
            </div>
        </td>
    )
            ...
            <div style={s} className="column-files">
                <ul className="list-unstyled" title={'共 ' + v.length + ' 个文件'}>
                    {v.map((item, idx) => {
                        ...
                        return <li key={'k-' + item} className="text-truncate"><a title={fileName}
                                                                                  onClick={(e) => CellRenders.clickPreview(item, null, e)}>{fileName}</a>
                        </li>
                    })}
                </ul>
            </div>
...
CellRenders.addRender('URL', function (v, s, k) {
    return (
        <td key={k}>
            <div style={s} title={v}>
                <a href={rb.baseUrl + '/commons/url-safe?url=' + $encode(v)} className="column-url" target="_blank"
                   rel="noopener noreferrer" onClick={(e) => $stopEvent(e)}>{v}</a>
            </div>
        </td>
    )
})
...
CellRenders.addRender('PHONE', function (v, s, k) {
    return (
        <td key={k}>
            <div style={s} title={v}><a href={'tel:' + v} className="column-url" onClick={(e) => $stopEvent(e)}>{v}</a>
            </div>
        </td>
    )
})
            ...
            <div className="row rb-datatable-footer">
                <div className="col-12 col-md-4">
                    <div className="dataTables_info" key="page-rowsTotal">
                        ...
                        <span className="mr-2">已选中 {this.state.selectedTotal} 条.</span>}
                        {this.state.rowsTotal > 0 && <span>共 {this.state.rowsTotal} 条数据</span>}
                    </div>
                </div>
                <div className="col-12 col-md-8">
                    <div className="float-right paging_sizes">
                        <select className="form-control form-control-sm" title="每页显示" onChange={this.setPageSize}
                                value={this.state.pageSize || 20}>
                    ...
                    <div className="float-right dataTables_paginate paging_simple_numbers">
                        <ul className="pagination">
                            {this.state.pageNo > 1 && <li className="paginate_button page-item"><a className="page-link"
                                                                                                   onClick={() => this.prev()}><span
                                className="icon zmdi zmdi-chevron-left"></span></a></li>}
                            {pages.map((item, idx) => {
                                if (item === '.') return <li key={`pnx-${idx}`}
                                                             className="paginate_button page-item disabled"><a
                                    className="page-link">...</a></li>
                                else return <li key={`pn-${item}`}
                                                className={'paginate_button page-item ' + (this.state.pageNo === item && 'active')}>
                                    <a className="page-link" onClick={this.goto.bind(this, item)}>{item}</a></li>
                            })}
                            {this.state.pageNo !== this.__pageTotal &&
                            <li className="paginate_button page-item"><a className="page-link"
                                                                         onClick={() => this.next()}><span
    ...
    showAdvFilter(id, useCopyId) {
        ...
        if (!id) {
            ...
            this.__getFilter(id, (res) => {
                renderRbcomp(<AdvFilter {...props} title="修改查询条件" filter={res.filter} filterName={res.name}
                                        shareTo={res.shareTo}/>)
            })
        }
    },
...
class RbViewModal extends React.Component {
            ...
            <div className="modal rbview" ref={(c) => this._rbview = c}>
                <div className="modal-dialog">
                    <div className="modal-content" style={{width: this.mcWidth + 'px'}}>
                        <div
                            className={'modal-body iframe rb-loading ' + (this.state.inLoad === true && 'rb-loading-active')}>
                            <iframe ref={(c) => this._iframe = c} className={this.state.isHide ? 'invisible' : ''}
                                    src={this.state.showAfterUrl || 'about:blank'} frameBorder="0"
                                    scrolling="no"></iframe>
                            <RbSpinner/>
                        </div>
                    </div>
    ...
    static create(props, subView) {
        ...
        if (subView) {
            renderRbcomp(<RbViewModal url={viewUrl} disposeOnHide={true} id={props.id}
                                      subView={true}/>, null, function () {
...
const ChartsWidget = {
    ...
    showChartSelect: function () {
        ...
        renderRbcomp(<ChartSelect select={(c) => this.renderChart(c, true)}
                                  entity={wpc.entity[0]}/>, null, function () {
