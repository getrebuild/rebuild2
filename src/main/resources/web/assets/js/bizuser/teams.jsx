...
class MemberList extends React.Component {
    ...
    render() {
        if (this.state.members && this.state.members.length === 0) return <div className="list-nodata"><span
            className="zmdi zmdi-info-outline"></span><p>请添加成员</p></div>
            ...
            {(this.state.members || []).map((item) => {
                return <tr key={`member-${item[0]}`}>
                    ...
                    <td className="actions">
                        <a className="icon" title="移除" onClick={() => this._removeMember(item[0])}><i
...
$(document).ready(() => {
    ...
    $('.nav-tabs a:eq(1)').click(() => {
        if (!memberList) renderRbcomp(<MemberList id={teamId}/>, 'tab-members', function () {
            memberList = this
        })
    })
    $('.J_add-slave').off('click').click(() => renderRbcomp(<MemberAddDlg id={teamId}
                                                                          call={() => memberList && memberList.loadMembers()}/>))
