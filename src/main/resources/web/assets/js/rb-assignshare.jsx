...
class DlgAssign extends RbModalHandler {
            ...
            <RbModal title={this.specs[1]} ref={(c) => this._dlg = c}>
                <div className="form">
                    ...
                    {this.state.cascadesShow !== true ? (
                        <div className="form-group row">
                            <div className="col-sm-7 offset-sm-3"><a href="#"
                                                                     onClick={this._showCascade}>同时{this.specs[1]}关联记录</a>
                            </div>
                        </div>
                    ) : (<div className="form-group row">
                    ...
                    <div className="form-group row footer">
                        <div className="col-sm-7 offset-sm-3" ref={(c) => this._btns = c}>
                            <button className="btn btn-primary btn-space" type="button" data-loading-text="请稍后"
                                    onClick={() => this.post()}>确定
                            </button>
    ...
    _useUserSelector() {
        return <UserSelector hideDepartment={true} hideRole={true} hideTeam={true} multiple={false}
    ...
    post() {
        ...
        if (!users || users.length === 0) {
            RbHighbar.create('请选择' + this.specs[1] + '给谁');
            return
        }
    ...
    static create(props) {
        ...
        else renderRbcomp(<DlgAssign {...props} />, null, function () {
            that.__HOLDER = this
        })
    }
}
...
class DlgShare extends DlgAssign {
    ...
    static create(props) {
        ...
        else renderRbcomp(<DlgShare {...props} />, null, function () {
            that.__HOLDER2 = this
        })
    }
}
...
class DlgUnshare extends RbModalHandler {
                        ...
                        <div className="col-sm-7">
                            <div className="mt-1">
                                <label
                                    ...
                                    <input className="custom-control-input" name="whichUsers" type="radio"
    ...
    static create(props) {
        const that = this
        if (that.__HOLDER) that.__HOLDER.show(props)
        else renderRbcomp(<DlgUnshare {...props} />, null, function () {
            that.__HOLDER = this
        })
    }
}
...
class DlgShareManager extends RbModalHandler {
        ...
        return (
            <RbModal title={(this.props.unshare === true ? '管理' : '查看') + '共享用户'} ref={(c) => this._dlg = c}>
                <div className="sharing-list">
                    <ul className="list-unstyled list-inline">
                        {(this.state.sharingList || []).map((item) => {
                            return (
                                <li className={`list-inline-item ${this.state.selectedAccess.includes(item[1]) ? 'active' : ''}`}
                                    key={`access-${item[1]}`}>
                ...
                <div className="dialog-footer" ref={(c) => this._btns = c}>
                    {this.props.unshare === true && <button className="btn btn-primary btn-space" type="button"
                                                            onClick={() => this.post()}>取消共享</button>}
                    <button className="btn btn-secondary btn-space" type="button" onClick={() => this.hide()}>取消
                    </button>
                </div>
            </RbModal>
        )
    ...
    static create(id, unshare) {
        ...
        const that = this
        if (that.__HOLDER) that.__HOLDER.show(props)
        else renderRbcomp(<DlgShareManager {...props} />, null, function () {
            that.__HOLDER = this
        })
    }
}