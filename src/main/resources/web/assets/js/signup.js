...
$(document).ready(function () {
  $('.J_vcode-btn').click(function () {
    ...
    if (!email) return RbHighbar.create('请输入邮箱')
  ...
  $('.J_confirm-btn').click(function () {
    const fullName = $val('#sFullName'),
      ...
      email = $val('#sEmail'),
      vcode = $val('#sVcode')
    if (!fullName) return RbHighbar.create('请输入姓名')
    if (!name) return RbHighbar.create('请输入用户名')
    if (!email) return RbHighbar.create('请输入邮箱')
    if (!vcode) return RbHighbar.create('请输入邮箱验证码')
