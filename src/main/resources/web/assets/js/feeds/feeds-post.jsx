        ...
        return <div className="feeds-post">
            <ul className="list-unstyled list-inline mb-1 pl-1" ref={(c) => this._activeType = c}>
                <li className="list-inline-item">
                    <a onClick={() => this.setState({type: 1})}
                       className={`${activeType === 1 ? activeClass : ''}`}>动态</a>
                </li>
                <li className="list-inline-item">
                    <a onClick={() => this.setState({type: 2})}
                       className={`${activeType === 2 ? activeClass : ''}`}>跟进</a>
                </li>
                <li className="list-inline-item">
                    <a onClick={() => this.setState({type: 4})}
                       className={`${activeType === 4 ? activeClass : ''}`}>日程</a>
                </li>
                {rb.isAdminUser && <li className="list-inline-item">
                    <a onClick={() => this.setState({type: 3})}
                       className={`${activeType === 3 ? activeClass : ''}`}>公告</a>
                </li>
                }
            </ul>
            ...
            <div className="mt-3">
                ...
                <div className="float-right mr-4">
                    <div className="btn-group" style={{border: '0 none'}}>
                        <button className="btn btn-scope btn-link" data-toggle="dropdown"
                                ref={(c) => this._scopeBtn = c}><i className="icon up-1 zmdi zmdi-chart-donut"/>公开
                        </button>
                        <div className="dropdown-menu dropdown-menu-right">
                            <a className="dropdown-item" onClick={this._selectScope} data-scope="ALL" title="全部可见"><i
                                className="icon up-1 zmdi zmdi-chart-donut"/>公开</a>
                            <a className="dropdown-item" onClick={this._selectScope} data-scope="SELF" title="仅自己可见"><i
                                className="icon up-1 zmdi zmdi-lock"/>私密</a>
                            <a className="dropdown-item" onClick={this._selectScope} data-scope="GROUP"
                               title="团队成员可见"><i className="icon up-1 zmdi zmdi-accounts"/>团队</a>
                        </div>
                    </div>
                </div>
    ...
    _selectScope = (e) => {
        ...
        this.setState({scope: target.dataset.scope}, () => {
            ...
            if (this.state.scope === 'GROUP') {
                ...
                else renderRbcomp(<SelectGroup call={this._renderGroupScope}/>, null, function () {
...
class FeedsEditor extends React.Component {
    ...
    constructor(props) {
        ...
        for (let k in EMOJIS) {
            const item = EMOJIS[k]
            this.__es.push(<a key={`em-${item}`} title={k} onClick={() => this._selectEmoji(k)}><img
                src={`${rb.baseUrl}/assets/img/emoji/${item}`}/></a>)
        }
    }
    ...
    render() {
        ...
        <textarea ref={(c) => this._editor = c} placeholder={this.props.placeholder} maxLength="2000"
                ...
                <div className="action-btns">
                    <ul className="list-unstyled list-inline m-0 p-0">
                        <li className="list-inline-item use-dropdown">
                            <a onClick={() => this.setState({renderEmoji: true})} title="表情" data-toggle="dropdown"><i
                                className="zmdi zmdi-mood"/></a>
                            <div className="dropdown-menu">
                                {this.state.renderEmoji && <div className="emoji-wrapper">{this.__es}</div>}
                            </div>
                        </li>
                        <li className="list-inline-item">
                            <UserSelector hideDepartment={true} hideRole={true} hideTeam={true} hideSelection={true}
                                          multiple={false}
                                          ref={(c) => this._UserSelector = c}
                                          compToggle={<a title="@用户" data-toggle="dropdown"><i
                                              className="zmdi at-text">@</i></a>}
                                          onSelectItem={this._selectAtUser}/>
                        </li>
                        <li className="list-inline-item">
                            <a title="图片" onClick={() => this._imageInput.click()}><i
                                className="zmdi zmdi-image-o"/></a>
                        </li>
                        <li className="list-inline-item">
                            <a title="附件" onClick={() => this._fileInput.click()} style={{marginLeft: -5}}><i
                                className="zmdi zmdi-attachment-alt zmdi-hc-rotate-45"/></a>
                        </li>
                    </ul>
                </div>
            </div>
            ...
            <ScheduleOptions ref={(c) => this._scheduleOptions = c} initValue={this.state.contentMore}
                             contentMore={this.state.contentMore}/>
            }
                    ...
                    {(this.state.files || []).map((item) => {
                        ...
                        return (
                            <div key={'file-' + item} className="img-thumbnail" title={fileName}>
                                ...
                                <span>{fileName}</span>
                                <b title="移除" onClick={() => this._removeFile(item)}><span
                                    className="zmdi zmdi-close"></span></b>
                            </div>
                        )
                    })}
                </div>
            ...
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content">
                    <div className="modal-header pb-0">
                        <button className="close" type="button" onClick={this.hide}><i className="zmdi zmdi-close"/>
                        </button>
                    </div>
                    <div className="modal-body">
                        <h5 className="mt-0 text-bold">选择团队</h5>
                        {(this.state.groups && this.state.groups.length === 0) &&
                        <p className="text-muted">你未加入任何团队</p>}
                        <div>
                            <ul className="list-unstyled">
                                {(this.state.groups || []).map((item) => {
                                    return <li key={'g-' + item.id}><a className="text-truncate"
                                                                       onClick={() => this._handleClick(item)}>{item.name}<i
                                        className="zmdi zmdi-check"></i></a></li>
                                })}
                            </ul>
                        </div>
                    </div>
...
class AnnouncementOptions extends React.Component {
            ...
            <dl className="row mb-1">
                ...
                <dd className="col-12 col-lg-9 mb-0" ref={(c) => this._showWhere = c}>
                    <label className="custom-control custom-checkbox custom-control-inline">
                        <input className="custom-control-input" name="showOn" type="checkbox" value={1}
                               disabled={this.props.readonly}/>
                        <span className="custom-control-label">动态页</span>
                    </label>
                    <label className="custom-control custom-checkbox custom-control-inline">
                        <input className="custom-control-input" name="showOn" type="checkbox" value={2}
                               disabled={this.props.readonly}/>
                        <span className="custom-control-label">首页</span>
                    </label>
                    <label className="custom-control custom-checkbox custom-control-inline">
                        <input className="custom-control-input" name="showOn" type="checkbox" value={4}
                               disabled={this.props.readonly}/>
                        <span className="custom-control-label">登录页 <i className="zmdi zmdi-help zicon down-3"
                                                                      data-toggle="tooltip" title="选择登录页公示请注意不要发布敏感信息"/></span>
                    </label>
                </dd>
            </dl>
            <dl className="row">
    ...
    val() {
        let where = 0
        $(this._showWhere).find('input:checked').each(function () {
            where += ~~$(this).val()
        })
...
class ScheduleOptions extends React.Component {
    ...
    val() {
        let remind = 0
        $(this._scheduleRemind).find('input:checked').each(function () {
            remind += ~~$(this).val()
        })
